...
app.post('/capture-context', async (req, res) => {
    try {
        const env = req.body.env || 'prod'; // Default to prod just in case
        ...
        // Logic to switch between DEV and PROD
        let url, token;
        if (env === 'dev') {
            url = 'https://merchant-order-token.baelab.net/v1/payments/capture-context';
            token = 'MDAxMTUwOTkyOilFVj02UU1GX2RDVmdUYW4yUEd+NnBYaCNzRUtrbg=='; // Your Dev Token
        } else {
            url = 'https://merchant-order-token.bankaletihad.com/v1/payments/app2/capture-context';
            token = process.env.MY_TOKEN; // Your Live Prod Token from Render
        }
